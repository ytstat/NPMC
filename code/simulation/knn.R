...
filename = paste("/burg/home/yt2661/projects/NPMC/experiment/simulation/knn/result/", seed, ".RData", sep = "")
...
for (i in 1:length(n.list)) {
  ...
  t1.CX <- proc.time()
  fit.npmc.CX <- try(npcs(x, y, algorithm = "CX", classifier = "knn", w = w, alpha = alpha, k = floor(sqrt(n.list[i])/3)))
  ...
  t1.ER <- proc.time()
  fit.npmc.ER <- try(npcs(x, y, algorithm = "ER", classifier = "knn", w = w, alpha = alpha, refit = FALSE, k = floor(sqrt(n.list[i])/3)))
  ...
  fit.vanilla <- knn3(x = x, y = factor(y), k = floor(sqrt(n.list[i])/3))
  ...
  t1.mnpo <- proc.time()
  fit.mnpo <- mnpo(x, y, classifier = "knn", alpha = alpha, w = w, increment = 0.01, k = floor(sqrt(n.list[i])/3))
  if (is.list(fit.mnpo)) {
...
for (i in 1:length(n.list)) {
  ...
  fit.npmc.CX<- try(npcs(x, y, algorithm = "CX", classifier = "knn", w = w, alpha = alpha, k = floor(sqrt(n.list[i])/4)))
  fit.npmc.ER <- try(npcs(x, y, algorithm = "ER", classifier = "knn", w = w, alpha = alpha, refit = TRUE, k = floor(sqrt(n.list[i])/4)))
  fit.vanilla <- knn3(x = x, y = factor(y), k = floor(sqrt(n.list[i])/4))
...
for (i in 1:length(n.list)) {
  ...
  t1.CX <- proc.time()
  fit.npmc.CX <- try(npcs_confusion(x, y, algorithm = "CX", classifier = "knn", w = w, alpha = alpha, k = floor(sqrt(n.list[i])/3)))
  ...
  t1.ER <- proc.time()
  fit.npmc.ER <- try(npcs_confusion(x, y, algorithm = "ER", classifier = "knn", w = w, alpha = alpha, refit = FALSE, k = floor(sqrt(n.list[i])/3)))
  ...
  t1.mnpo <- proc.time()
  fit.mnpo <- mnpo_confusion(x, y, classifier = "knn", alpha = alpha, w = w, increment = 0.01, k = floor(sqrt(n.list[i])/3))
  if (is.list(fit.mnpo)) {
    y.pred.mnpo <- predict_mnpo(fit.mnpo, x.test)
  }
  t2.mnpo <- proc.time()
