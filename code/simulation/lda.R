...
filename = paste("/burg/home/yt2661/projects/NPMC/experiment/simulation/lda/result/", seed, ".RData", sep = "")
...
for (i in 1:length(n.list)) {
  ...
  t1.CX <- proc.time()
  fit.npmc.CX <- try(npcs(x, y, algorithm = "CX", classifier = "lda", w = w, alpha = alpha))
  ...
  t1.ER <- proc.time()
  fit.npmc.ER <- try(npcs(x, y, algorithm = "ER", classifier = "lda", w = w, alpha = alpha, refit = FALSE))
  ...
  fit.vanilla <- lda(x = x, grouping = factor(y))
  y.pred.vanilla <- predict(fit.vanilla, x.test)$class
  ...
  t1.mnpo <- proc.time()
  fit.mnpo <- mnpo(x, y, classifier = "lda", alpha = alpha, w = w, increment = 0.01)
  if (is.list(fit.mnpo)) {
...
for (i in 1:length(n.list)) {
  ...
  t1.CX <- proc.time()
  fit.npmc.CX <- try(npcs_confusion(x, y, algorithm = "CX", classifier = "lda", w = w, alpha = alpha))
  ...
  t1.ER <- proc.time()
  fit.npmc.ER <- try(npcs_confusion(x, y, algorithm = "ER", classifier = "lda", w = w, alpha = alpha, refit = FALSE))
  ...
  t1.mnpo <- proc.time()
  fit.mnpo <- mnpo_confusion(x, y, classifier = "lda", alpha = alpha, w = w, increment = 0.01)
  if (is.list(fit.mnpo)) {
    y.pred.mnpo <- predict_mnpo(fit.mnpo, x.test)
  }
  t2.mnpo <- proc.time()
