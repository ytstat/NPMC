#!/bin/sh
#
# run "dementia.R" using the script
#
#SBATCH --account=stats         # Replace ACCOUNT with your group account name
#SBATCH --job-name=dementia    # The job name.
...
R CMD BATCH --no-save --vanilla dementia.R /out/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.txt # run "dementia.R" and save the output information
